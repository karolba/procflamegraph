...
fn usage(application_name: &str, err: Option<&str>) {
    ...
    let _ = writeln!(b, "         --capture-stdout    - show stdout from all child processes");
    let _ = writeln!(b, "         --no-capture-stdout - don't show stdout from all child processes processes (default)");
...
pub(crate) fn parse_args() -> Args {
    ...
    args.our_name = parser
        .bin_name()
        .unwrap_or("procflamegraph")
        .to_string();

    while let Some(arg) = parser.next().unwrap_or_else(|err| {
        usage(&args.our_name, Some(&err.to_string()));
        exit(1)
    }) {
        match arg {
            ...
            Long("capture-stdout") => args.capture_stdout = true,
            Long("no-capture-stdout") => args.capture_stdout = false,
            ...
            Short('o') | Long("output") => {
                args.output_file = Some(parser.value().unwrap_or_else(|err| {
                    usage(&args.our_name, Some(&err.to_string()));
                    exit(1)
                }));
            }
