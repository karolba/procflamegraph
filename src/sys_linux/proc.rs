use crate::sys_linux::{fd::read_to_maybe_uninit, macros::eintr_repeat};
use nix::{errno::Errno,
          fcntl::{OFlag, open},
          sys::stat::{FileStat, Mode, stat},
          unistd::Pid};
use std::{ffi::CStr,
          mem::MaybeUninit,
          os::fd::{AsFd, OwnedFd}};
    ...
    loop {
        match eintr_repeat!(read_to_maybe_uninit(fd.as_fd(), data.spare_capacity_mut())) {
            ...
            Ok(num) => {
                ...
                data.reserve(if read_was_exact {
                    read_options.expand_buffer_by
                } else {
                    read_options.expand_buffer_on_partial_read
                } as usize);
            }
            Err(err) => Err(err)?,
        };
...
pub(crate) fn process_cmdline(pid: Pid) -> Result<(Vec<u8>, OwnedFd), Errno> {
    const PATH_SIZE: usize = c"/proc/-2147483648/cmdline"
        .to_bytes_with_nul()
        .len();
    ...
    let size: usize = unsafe {
        ...
        libc::snprintf(path.as_mut_ptr() as *mut libc::c_char, PATH_SIZE, c"/proc/%d/cmdline".as_ptr(), pid.as_raw() as libc::c_int)
    } as usize
...
pub(crate) fn process_environ(pid: Pid) -> Result<(Vec<u8>, OwnedFd), Errno> {
    const PATH_SIZE: usize = c"/proc/-2147483648/environ"
        .to_bytes_with_nul()
        .len();
    let mut path: [MaybeUninit<u8>; PATH_SIZE] = [const { MaybeUninit::uninit() }; PATH_SIZE];
    ...
    let size: usize = unsafe {
        // The easiest way to fill up a c_char array on the stack in rust
        libc::snprintf(path.as_mut_ptr() as *mut libc::c_char, PATH_SIZE, c"/proc/%d/environ".as_ptr(), pid.as_raw() as libc::c_int)
    } as usize
        + 1; // +1 for the null terminator
...
pub(crate) fn process_stat(pid: Pid) -> Result<(Vec<u8>, OwnedFd), Errno> {
    const PATH_SIZE: usize = c"/proc/-2147483648/stat"
        .to_bytes_with_nul()
        .len();
    let mut path: [MaybeUninit<u8>; PATH_SIZE] = [const { MaybeUninit::uninit() }; PATH_SIZE];
    ...
    let size: usize = unsafe {
        // The easiest way to fill up a c_char array on the stack in rust
        libc::snprintf(path.as_mut_ptr() as *mut libc::c_char, PATH_SIZE, c"/proc/%d/stat".as_ptr(), pid.as_raw() as libc::c_int)
    } as usize
        + 1; // +1 for the null terminator
...
pub(crate) fn process_is_fd_a_pipe(process: Pid, child_fd: i32) -> bool {
    const PATH_SIZE: usize = c"/proc/-2147483648/fd/-2147483648"
        .to_bytes_with_nul()
        .len();
    ...
    const EXPECTED_PREFIX: [libc::c_char; 5] =
        [b'p' as libc::c_char, b'i' as libc::c_char, b'p' as libc::c_char, b'e' as libc::c_char, b':' as libc::c_char];
    let mut prefix: [libc::c_char; EXPECTED_PREFIX.len()] = Default::default();

    let res = eintr_repeat!(Errno::result(unsafe { libc::readlink(path.as_ptr(), prefix.as_mut_ptr(), prefix.len()) }));
    match res {
        Err(_) => false,
...
pub(crate) fn stat_process_exe(pid: Pid) -> nix::Result<FileStat> {
    const PATH_SIZE: usize = c"/proc/-2147483648/exe"
        .to_bytes_with_nul()
        .len();
    let mut path: [MaybeUninit<u8>; PATH_SIZE] = [const { MaybeUninit::uninit() }; PATH_SIZE];
    ...
    let size: usize = unsafe {
        // The easiest way to fill up a c_char array on the stack in rust
        libc::snprintf(path.as_mut_ptr() as *mut libc::c_char, PATH_SIZE, c"/proc/%d/exe".as_ptr(), pid.as_raw() as libc::c_int)
    } as usize
        + 1; // +1 for the null terminator
